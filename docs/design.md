...
| Parallax          | Mountains shift on scroll via `camY = scrollY / 1200` panning. Passive scroll sampling forwards the camera before the main animation callback; worker updates coalesce to the newest state once per requested frame. |
