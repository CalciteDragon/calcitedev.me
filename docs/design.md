...
- An open 1800×700 DOM/CSS world places three broad monitors directly over the site's existing mountain-and-stars background: gold Capstone Summit, cyan Keyboard Cove, and pink Robotics Outpost. There is no enclosing game window, HUD bar, local sky, destination navigation, horizontal crop, or side-scrolling camera. The monitor top edges are the island collision surfaces, and a CSS pixel explorer stands and jumps directly on the screens.
- Every monitor has rounded corners, a dark bezel, and a non-interactive scanline overlay. Redundant topic/status text above the media, caption/counter text below it, and the visible teleport prompt are omitted because the island copy and click behavior already identify the content. Multi-item galleries expose only circular previous/next arrows over the media. The active island intensifies its accent border/glow without adding more pane chrome; inactive panes remain full-surface teleport buttons with accessible labels.
- The explorer starts 58px inside the left edge of the middle Keyboard island while all three monitors remain in `STANDBY`. Arrow input can move the explorer without beginning media playback. The first W, A, S, or D press enables activation and activates whichever platform currently supports the explorer; afterward, standing on or teleporting to an island activates it normally.
- Activating an island lifts the monitor 8px with matching collision geometry. Activation persists for one second after the explorer leaves, so normal jumps do not flicker the island state. A real YouTube video is inserted only while its video record is active and starts muted through the privacy-enhanced embed; leaving long enough to deactivate destroys the iframe and stops playback. The player records YouTube time updates and resumes the next embed from the last saved whole-second timestamp. Multi-item photo/video topics slide to the next record every 5.2 seconds while active, with explicit previous/next buttons for manual control.
...
- The game is enhancement, not a content gate. At desktop widths, page-level WASD/arrow listeners keep movement working after teleporting or clicking elsewhere on the page; editable fields are left alone. There are no visible direction or jump buttons. Users can click anywhere on an inactive monitor to teleport directly onto it without moving focus; before the first WASD press, teleporting changes position but deliberately keeps every screen in standby. A subtle speech bubble beside the explorer says `try WASD` and disappears permanently for the component session when that first W, A, S, or D press also enables activation.
