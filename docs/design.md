...
- `prefers-reduced-motion` holds the explorer on a still sprite frame, disables the speech-bubble entrance and copy emphasis transitions, and stops automatic gallery advancement. User-driven desktop movement, facing, crouch/jump pose changes, click-to-teleport, manual gallery controls, and video controls remain available.
