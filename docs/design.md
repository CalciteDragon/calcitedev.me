...
- Manually browsing a gallery pauses its 5.2-second auto-advance until the next W/A/S/D press or a real island-activation change; opening a pop-out also pauses auto-advance until it closes.
- Activating an island lifts the monitor, collision surface, and its visible label/title/description together by 8px. Activation persists for one second after the explorer leaves, so normal jumps do not flicker the island state. A real YouTube video is inserted only while its video record is active and starts muted through the privacy-enhanced embed; leaving long enough to deactivate destroys the iframe and stops playback. A media record may provide an initial timestamp, with saved whole-second playback progress taking priority on later activations. The Robotics Outpost video starts at 3:03:36. Multi-item photo/video topics slide to the next record every 5.2 seconds while active, but pause that timer for as long as the current YouTube video reports that it is playing. Explicit previous/next buttons remain available for manual control.
- The same gallery timer also stays paused after manual previous/next navigation and while the pop-out is open; video playback and reduced-motion rules continue to take precedence.
