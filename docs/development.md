...
- The former Skills grid has been replaced by an open Extras platformer covering the Pineapple Expense capstone presentation, custom keyboard build, and Mochi 2026 robotics competition in that order. Three broad scanline monitors live directly over the site background and double as collision islands. The full 1800×700 level scales into the available desktop width with no camera, horizontal cutoff, or side scrolling; smaller widths switch to readable vertically stacked panes and an explorer prompt to try desktop or widen the window. The explorer starts near the left edge of the middle Keyboard island while every screen remains in standby; arrows can move immediately, but the first W/A/S/D press enables and performs the first activation. Page-level input, focus-independent click-to-teleport screens, an 8px active-platform lift, one-second deactivation grace period, timestamp-resuming YouTube playback, visible island copy, manual/automatic galleries, reduced-motion behavior, and labeled slots for the still-pending capstone/robotics assets are implemented. The former on-screen direction/jump controls have been removed.
...
- Static output is configured and verified: 129 tests pass and a production build prerenders four routes.
...
- Known repository gaps: the hero (7.39 kB), About (7.75 kB), project focus (6.59 kB), project selector (4.88 kB), and Extras platformer (6.15 kB) stylesheets exceed the 4 kB warning budget but remain below the 8 kB error limit; the simplified Extras media-screen stylesheet is now below the warning budget. Final capstone/robotics media, mountain DPR scaling, the broader responsive/accessibility audits, an og:image asset, and external deployment verification remain open.
...
- [x] `ExtraMediaScreenComponent` — feature-local rounded monitor for privacy-enhanced, timestamp-resuming video and image/placeholder gallery records
...
- [x] `extras.data.ts` — capstone, keyboard, and robotics topics; keyboard uses the supplied YouTube build while the remaining records expose labeled asset slots
...
- [x] Page-level WASD/arrow controls that remain active after teleporting or clicking elsewhere, plus focus-independent full-island click-to-teleport overlays; no on-screen direction/jump controls or destination navigation bar
- [x] Active screens rise 8px with matching collision surfaces and wait one second after the explorer leaves before deactivating
- [x] Start the explorer near the left edge of the middle Keyboard island with all screens in standby; the first WASD keypress removes the subtle `try WASD` bubble and enables/executes the first platform activation, while arrow-only movement remains inactive
...
- [x] `ExtraMediaScreenComponent` — rounded scanline monitors with visual active/standby states, privacy-enhanced muted YouTube playback that resumes from its last reported timestamp, placeholder/image states, and arrow-only overlay controls for multi-item galleries; redundant header/status and footer caption/counter chrome is removed
...
**Built:** `BackgroundSceneComponent` lives in `LayoutComponent`. `HomeComponent` reads `projectsData`, `extrasData`, `socialLinksData`, and `bioData` and passes them through signal inputs. The homepage order is Hero → About → Projects → Extras → Contact. Projects use `ProjectsSectionComponent` selection state, an overlapping `ProjectFocusStageComponent`, and `ProjectSelectorComponent`; Extras owns local player, responsive mode, delayed active-island, speech-prompt, saved-video-time, and gallery state; there is no project or Extras route. The August 14 verification passes 129 tests and prerenders four routes.
