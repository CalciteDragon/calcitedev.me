...
- The project display order now leads with Live Bingo, Pineapple Expense, Calcite Portfolio, and Mochi 2026; the browser platformer and the remaining builds follow. Each project-index card owns an individual scroll reveal while retaining its independent hover/press transform.
- The former Skills grid has been replaced by an open Extras platformer covering the Pineapple Expense capstone presentation, custom keyboard build, and Mochi 2026 robotics competition in that order. Three broad scanline monitors live directly over the site background and double as collision islands. The full 1880×820 level scales into the available desktop width with no camera, horizontal cutoff, or side scrolling; smaller widths switch to readable vertically stacked panes and an explorer prompt to try desktop or widen the window. The explorer is now a clean six-pose 8-bit counterpart to the hero, retaining his swept brown hair, glasses, blue headphones, dark hoodie, and cyan shoes in a 38×48 sprite strip with animated walking, facing, crouch, and airborne poses. Its collision box and all level geometry remain unchanged. The explorer starts near the left edge of the middle Keyboard island while every screen remains in standby; arrows can move immediately, while the first W/A/S/D press activates the supporting island and removes the hint. Pointer teleport activates an island even before that first keypress while preserving the `try WASD` prompt. Page-level input, focus-independent click-to-teleport screens, synchronized 8px monitor/copy/collision lift, one-second deactivation grace period, timestamp-resuming YouTube playback, always-hoverable gallery arrows that browse and teleport inactive islands, and reduced-motion behavior are implemented. Pineapple Expense follows the supplied IMG_1 → IMG_2 → rendered IMG_3 → IMG_4 → IMG_5 image order, while Robotics Outpost follows IMG_2 → IMG_1 → competition video → IMG_4 → IMG_3; the video begins at 3:03:36 on its first play. The former on-screen direction/jump controls have been removed.
- The August 15 pass removed timed gallery auto-advance entirely, along with its pause state and the video-playing signal chain that gated it. Slides now move only from the cursor arrows, the pop-out arrows, or a new pair of analog previous/next pushbuttons mounted flush on the top edge of each multi-slide island. The pads are derived from island geometry rather than stored in `extras-level.data.ts`, so the level editor is untouched; they are one-way top-collision platforms the explorer walks through and lands on from above. A landing advances exactly one slide, edge-triggered against the current supporting element so resting on a pressed pad never repeats it, and the cap plus the explorer sink 7px together. A rested-on pad also counts as its island's support, so standing there does not trigger the one-second deactivation. Keyboard Cove holds a single video and therefore has no pads. Each island also has an expand control that activates/teleports and opens a large viewport-centered pop-out with a stable 16:9 media frame and contain-fitting so every carousel image remains visible without resizing the window; the page-wide scrim dims the rest of the site, an overflow-only document lock keeps page scrolling disabled without changing the current scroll position, the pop-out keeps a softened scanline texture, the cyan outline and close control use a lower-opacity treatment, and pointer clicks on any Extras control release focus without affecting keyboard focus.
- The August 17 pass retitled the fifth project from `Pixel Quest` to `It's Never Just Black and White` and swapped its SVG placeholder for a real 1924×1039 in-game capture (`project-black-and-white.png`). The slug moved to `black-and-white` because the focus stage renders it verbatim as the `VISUAL_FEED` label, so a stale slug would have left the old name visible on screen; `projects.data.spec.ts` tracks the display order by slug and was updated with it. The record's `longDescription`, `description`, `tags`, and `githubUrl` still carry the original placeholder copy and are pending real project research. The same pass replaced the Minecraft Hide & Seek SVG with a rendered library scene (`project-hide-and-seek.jpg`), leaving the recursive Calcite Portfolio window as the last SVG preview; that record's copy was already researched and needed no change.
- The August 18 pass added carousel navigation to the project focus stage and a scanline treatment to the index previews. The arrows deliberately sit outside the pane rather than over its media, so they read as movement between projects instead of chrome belonging to the project on screen: the stage wraps its stacked articles in a `__panes` grid cell and places the two controls in named grid areas beside it — flanking from `md`, and a pager row above the card below `md` so they stay visible when the section is first reached. Moving them out of the article broke the `--project-accent` inheritance they had relied on, so the selected project's `glowColor` is now passed down explicitly and re-declared by a new `ProjectCarouselNavComponent`; extracting that component also returned the focus stylesheet to 7.08 kB. `ProjectFocusStageComponent` computes the wrapped targets and emits a slug through a `projectStepped` output, which `ProjectsSectionComponent.stepProject()` funnels into the existing `selectProject()` path with `focusDetails: false` so focus stays on the arrow and repeated steps keep working. The index previews gained a `repeating-linear-gradient` CRT overlay scoped to `.project-selector__visual`, sitting above the image and below the number/`ACTIVE` badges and easing from 0.85 to 0.5 opacity on hover/focus/active alongside the existing brightness lift; the focus stage image deliberately stays clean. Verified at 1440×1050, 820×640, and 420×700 against the running dev server — including a real-time-spaced click sequence that wrapped backward from 01 through 07 and 06 to 05 — with arrow wrapping, accent/label wiring, and the single-project fallback covered by new component tests. Note that several synchronous `.click()` calls in one task still advance only one step, because the stage's `selectedSlug` input propagates on change detection; real pointer input is always separated by a flush, so this is a test-harness artifact rather than a behavioural limit.
- The August 21 content pass added the playable build link `https://neverbnw.calcitedev.me` to the It's Never Just Black and White project record; the existing focus-stage “Visit project” action now opens the game in a new tab, and its preview status now reads `BROWSER GAME`.
- The About section now follows the hero and presents Tyler's supplied copy as a compiler-themed origin story: a larger intro card, production-command heading, and four-stage cyan → blue → violet → pink timeline. Passive rAF-coalesced scroll measurement keeps exactly one chapter active while frame-rate-independent damping smooths card tilt and rail progress; the rail glow fills from node 01 crossing 60% of viewport height to a fixed 46% of total page scroll, so it opens at the same on-screen point and completes at the same scroll offset on 1080p and 1440p alike, independent of the section's own progress. Its endpoints and color stops align to the numbered nodes, and number activation follows the same smoothed rail head. A development-only `?aboutDebug=scroll` HUD exposes live tuning values, and only the intro card uses scroll reveal. The content remains typed static data, and desktop/tablet/mobile renders were visually checked against the running site.
...
- The July 2026 polish pass added the original project-thumbnail treatment, hero UFO repositioning, mobile-local hero gradients, a global `:focus-visible` ring, skip-to-content link, drawer body-scroll lock with md auto-close, press-state compression, canvas reduced-motion still-frame + hidden-tab rAF pause, and static head metadata. The original pixel-C SVG favicon was later superseded by the faceted-C navbar artwork. Its card routing/filter UI was superseded by the August 13 project showcase.
...
- Known repository gaps: the hero (7.60 kB), About (7.75 kB), project focus (7.67 kB), Extras platformer (6.56 kB), project selector (5.40 kB), and navbar (4.69 kB) stylesheets exceed the 4 kB warning budget but remain below the 8 kB error limit. The August 18 carousel briefly pushed the project focus stylesheet to 7.84 kB, and moving the arrows outside the pane needed per-accent variant rules that would not fit; both were resolved by extracting `ProjectCarouselNavComponent`, whose own stylesheet sits below the warning budget; the new Extras media-pad, the simplified Extras media-screen, and the separate level-editor stylesheets remain below the warning budget. The platformer's slide pad has since been extracted into `extra-media-pad/` alongside the existing `extra-media-screen` and `extras-level-editor` siblings, which cut the platformer stylesheet from 7.99 kB to 6.56 kB and restored roughly 1.4 kB of headroom against the error budget. The pressed cap's 220ms ease-down is deliberate, not a missed instant-press: the platformer stylesheet carried a `transition-duration: 0ms` declaration that never applied (equal specificity, base rule written later), and it has been removed rather than made to work, so the cap keeps easing into its housing while the explorer's own drop stays instant. Mountain DPR scaling, the broader responsive/accessibility audits, an og:image asset, and external deployment verification remain open.
...
- [x] `NavbarComponent`:
  ...
  - Brand lockup on the left — supplied faceted-C PNG + `ALCITEdev.me`, with zero layout gap so the mark reads as the first letter and the full `dev.me` domain suffix uses the smaller mono style; the same artwork supplies the browser favicon
...
- [x] Continuous scroll-driven active chapter, damped card tilt and rail fill/head mapped from a 60%-viewport start line at node 01 to a fixed 46% page-scroll finish that behaves identically at 1080p and 1440p, number-aligned rail geometry/color stops, an intentional clean stop at node 04 with no lower tail, and a URL-gated development tuning HUD
...
- [x] Galleries advance only from deliberate input — cursor arrows, pop-out arrows, or the explorer landing on an island's analog slide pads; timed auto-advance was removed and decorative loops still stop under reduced motion
- [x] Island expand controls open centered media pop-outs with a stable 16:9 frame that contain-fits even the supplied 4:3 poster images, carousel arrows, and a top-right close button
...
- [x] `ProjectCarouselNavComponent` — one accent-aware carousel arrow; two instances flank the focus stage from `md` and form a pager row above it below `md`
